...
window.review2 = {
    ...
    fileUploader: {
        ...
        loaded: 0,
        max: editor.options.uploadImgMaxlen ? editor.options.uploadImgMaxlen : 20,
        status: 0, //上传状态
        files: [],
        init: function () {
            var that = this;
            ...
            $('.upl-t').find('input[type=file]').on('change', function () {
                ...
                var maxSize = editor.options.uploadImgMaxSizeM ? editor.options.uploadImgMaxSizeM : 10
        ...
        upload: function (file, imgLen) {
            ...
            $.ajax({
                type: 'post',
                url: editor.options.uploadImgParamAddr ? editor.options.uploadImgParamAddr : 'http://up9.pccoo.cn/upload.ashx?filesrc=madmin',
                data: formdata,
                async: false,
                contentType: false,
                processData: false,
                ...
                success: function (data, texts, xhr) {
                    ...
                    // } else {
                        console.log(data)
                        var html = $('<li class="onc on"><img  src="' + data + '" alt="图片show"></li>')
                        $('#imagelist').append(html)
                        if (imgLen == 0) {
                            var img = $('#imagelist').find('li:last-of-type img')
                            img.load(function () {
                                $('.upload-pre').text('上传已完成')
                            })
                        }
                        review2.insertimageList.push(data)
                    // }
                }
            })
        }
    ...
    getInsertList() {
        var arr = [];
        for (var i = 0; length = review2.insertimageList.length, i < length; i++) {
            arr.push({
                src: review2.insertimageList[i],
                width: '',
                height: ''
            })
        }
...
(function () {
        ...
        dialog.onok = function () {
            ...
            switch (id) {
                case 'remote':
                    ...
                    break;
                case 'upload':
                    list = window.review2.getInsertList();
                    break;
                case 'online':
    ...
    RemoteImage.prototype = {
        ...
        initContainer: function () {
            ...
            var img = editor.selection.getRange().getClosedNode();
            if (img) {
                if (img.nodeName == 'IMG' || img.nodeName == 'img') {
                    this.setImage(img);
                }
            }
        },
