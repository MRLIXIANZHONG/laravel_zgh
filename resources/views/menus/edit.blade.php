...
@section('content')
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" value="{{!empty($menu['title']) ? $menu['title'] : ''}}" name="title" required lay-verify="title" placeholder="请输入名称" autocomplete="off" class="layui-input">
        </div>
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="number" value="{{!empty($menu['order']) ? $menu['order'] : '0'}}" name="order" required lay-verify="order" placeholder="请输入数字" autocomplete="off" class="layui-input">
        </div>
...
{{--        <div class="layui-input-block">--}}
{{--            <input type="hidden" name="icon" value="{{!empty($menu['icon']) ? $menu['icon'] : ''}}" lay-verify="required" placeholder="请选择图标" autocomplete="off" class="layui-input" readonly>--}}
...
{{--    </div>--}}
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" value="{{!empty($menu['uri']) ? $menu['uri'] : ''}}" name="uri" required lay-verify="uri" placeholder="请输入URL,样式如:/fzs" autocomplete="off" class="layui-input">
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            @foreach($roles as $role)
                <input type="checkbox" value="{{$role['id']}}" required {{isset($menu['roleIds'])&&in_array($role['id'],$menu['roleIds'])?'checked':''}} lay-filter="roles_check" name="roles[]" title="{{$role['name']}}">
            @endforeach
        </div>
    </div>
@endsection
