...
@section('header')
    <div class="layui-inline">
        <div class="layui-btn layui-btn-small layui-btn-warm hidden-xs fresh">
            <a href="{{url('/admin/candidate_craftsmans')}}"><i class="layui-icon">&#x1002;</i></a></div>
    </div>
    {{--<div class="layui-inline">--}}
    ...
    {{--</div>--}}
    <form action = "{{url('admin/candidate_craftsmans')}}" method="get">
    ...
    @if(in_array($role, [1,4]))
        <div class="layui-inline">
            <a href="{{url('admin/candidate_craftsmans/export')}}" style="display: inline-block;width: 64px; height: 38px;line-height: 38px;background-color: #BC0000;text-align: center; color: #ffffff;">导出</a>
        </div>
    @endif
        ...
        <thbody>
            @foreach($craftsmans as $craftsman)
                <tr>
                    ...
                    <td>
                        <div class="layui-inline">
                            <a href="{{ url('/admin/candidate_craftsmans/'. $craftsman->id ) }}" class="layui-btn layui-btn-small layui-btn-blue edit-btn" role="button" class="layui-icon">
                                <i class="layui-icon">&#xe615;</i>详情</a>

                            @if($role === 1)
                                <a href="{{ url('/admin/candidate_craftsmans/'. $craftsman->id .'/edit') }}" class="layui-btn layui-btn-small layui-btn-blue edit-btn" role="button" class="layui-icon">
                                    <i class="layui-icon">&#xe642;</i>编辑</a>

                                <button type="button" class="layui-btn layui-btn-small layui-btn-danger del-btn" @click="deleteCandidateCraftsman({{$craftsman['id']}})"
                                ><i class="layui-icon">&#xe640;</i>删除</button>
                            @endif
                            @if(in_array($role,[1,4,2]))
                                <button type="button" class="layui-btn layui-btn-small layui-btn-blue del-btn" @click="check({{$craftsman['id']}})"><i class="layui-icon">&#xe642;</i>审核</button>
                                <button type="button" class="layui-btn layui-btn-small layui-btn-blue del-btn" @click="reject({{$craftsman['id']}})"><i class="layui-icon">&#x1006;</i>驳回</button>
                            @endif
                            @if(in_array($role, [1,4]))
                                <button type="button" class="layui-btn layui-btn-small layui-btn-blue del-btn" @click="set_craftsman({{$craftsman['id']}})"><i class="layui-icon">&#xe615;</i>评选</button>
                            @endif
                        </div>
                    </td>
                </tr>
                ...
                deleteCandidateCraftsman(id) {
                    layer.msg('您确认要删除吗？', {
                        ...
                        , yes: function (index) {
                            ...
                            $.ajax({
                                url: "{{url('/admin/candidate_craftsmans')}}/" + id,
                                data: {
                                    _token: '{{csrf_token()}}'
                                },
                                type: 'delete',
